...
class Mstudents extends CI_Model
{
	...
	public function listbycourse($course=0)
	{
	...
	public function listbystudentsbypenalty($course_id='')
	{
		...
		//return $this->db->get('students')->result();
		$this->db
			->select('students.*,total_late_penalty')
			->from('students')
			->join('v_penalty_total','students.code = v_penalty_total.student_code','LEFT');
			$query = $this->db->get();
			if($result = $query->result()){
				$data = array();
				foreach ($result as $key => $value) {
	...
	public function getCourse($student_id)
	{
		...
		//echo "$student_id";
		$this->db
			->select('year,section,grade,course.course_sub_title,course_id')
